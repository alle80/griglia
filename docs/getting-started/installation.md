...
!!! warning "Back up your instruction files before this step"
    ...
    scripts/sync-context.py --backup                   # copies them to docs/context-originals/
    ...
    sync. To undo: `scripts/sync-context.py --restore` puts the originals back, and switching off «Generate
...
```bash
scripts/sync-context.py            # write CLAUDE.md and AGENTS.md if the content changed
scripts/sync-context.py --check    # exit 1 if they are out of date (useful in CI)
```
...
| `CLAUDE.md` came back different | the board generates it | edit the blocks on `/context`, or `scripts/sync-context.py --restore` |
