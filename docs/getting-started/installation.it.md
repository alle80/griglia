...
!!! warning "Fai il backup dei file di istruzioni prima di questo passo"
    ...
    python3 scripts/sync-context.py --backup                   # li copia in docs/context-originals/
    ...
    prima sincronizzazione. Per tornare indietro: `python3 scripts/sync-context.py --restore` rimette gli originali, e
...
```bash
python3 scripts/sync-context.py            # scrive CLAUDE.md e AGENTS.md se il contenuto è cambiato
python3 scripts/sync-context.py --check    # esce con 1 se non sono aggiornati (utile in CI)
```
...
| `CLAUDE.md` è tornato diverso | lo genera la board | modificare i blocchi su `/context`, oppure `python3 scripts/sync-context.py --restore` |
