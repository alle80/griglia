# Domande frequenti

Risposte brevi alle domande più comuni: prima di installare Griglia e nei primi giorni di uso della board.
Le versioni lunghe stanno nelle pagine collegate da ogni risposta. Se una parola non ti è familiare, la
definisce il [glossario](glossary.md).

## Prima di installarla

### Griglia funziona con il mio agente di coding?

Griglia non si integra con un agente specifico: il contratto sono due comandi artisan e un file di istruzioni.
Qualunque agente CLI capace di eseguire `php artisan griglia:check` e di leggere un file Markdown può usare la
board — Claude Code, Codex CLI, Gemini CLI e altri sono già stati usati così. `vendor:publish --tag=griglia-agents`
scrive le istruzioni che l'agente legge. Vedi [il lato agente](agent/index.md).

### È un'applicazione a parte?

No. Griglia è un package Composer che si installa in un'applicazione Laravel esistente: `composer require`,
`php artisan migrate`, e la board vive nella tua app, con i tuoi utenti e il tuo database. Vedi il
[tutorial di installazione](getting-started/installation.md).

### Servono una chiave AI o una connessione a internet?

No. La board, il flusso con l'agente, le notifiche e i temi funzionano senza alcun provider AI. Le descrizioni
delle immagini, i piani costruiti da un prompt e la trascrizione vocale sono opzionali e si attivano solo se
`laravel/ai` è configurato. Vedi [funzioni AI](features/ai.md).

### Serve Docker?

No. Griglia gira dove gira la tua applicazione Laravel — `composer dev`, `php artisan serve`, Apache, nginx con
PHP-FPM o un container. Gli script sull'host se ne accorgono da soli: usano il container quando c'è e
altrimenti `php artisan`. Vedi [Usare Griglia senza Docker](getting-started/without-docker.md).

### Servono Node, Vite o Tailwind?

Non nella configurazione predefinita: Griglia distribuisce CSS e JavaScript già compilati. Se preferisci
compilare il package con la tua build Tailwind, imposta `GRIGLIA_ASSETS=vite` e segui
[asset front-end](getting-started/assets.md).

### È abbastanza stabile da usarla?

La usa ogni giorno il suo autore, ha una suite di test che gira a ogni push e segue il versionamento semantico
— ma è ancora `0.x`: le versioni minori possono cambiare comportamento, e il changelog dice quando. Vedi il
[runbook di aggiornamento](operations/upgrading.md) e la [governance e politica di supporto](contributing/governance.md).

## Usare la board

### Ho aggiunto un task, ma `griglia:check` dice che non c'è niente da fare

Due motivi consueti, entrambi voluti:

- **Il task è ancora *in attesa***. L'agente vede solo ciò che gli hai passato: tocca una volta il pallino di
  stato, così diventa *da lavorare*. Tutto il resto della board resta tuo, e l'agente non lo tocca.
- **Il task è ancora *in attesa* in un'altra lista**. Solo la **lista dell'agente** — `agent_list` nella
  configurazione, `dev` di default, cambiabile con `GRIGLIA_AGENT_LIST` o dalle impostazioni — è il canale che
  l'agente legge dall'alto in basso. Nelle altre tue liste non succede niente finché non passi un task:
  mettilo *da lavorare* e `griglia:check` lo mostra sotto `📋 List «…»`, dopo la lista dell'agente e dopo i
  piani. Per le richieste di tutti i giorni usa la lista dell'agente: lì decidi la priorità col trascinamento.

I piani funzionano allo stesso modo: i loro task compaiono dopo la lista dell'agente. Vedi
[usare la board](board/usage.md) e [piani](features/plans.md).

### Posso usarla senza login?

`GRIGLIA_MODE=local` toglie l'autenticazione e rende le liste globali — per una board sulla tua macchina, in
ascolto su `127.0.0.1`. Non esporre mai la modalità local su una rete. Su un server l'accesso è deciso da
`canAccessGriglia()` o `GRIGLIA_ACCESS_GATE`, e le pagine amministrative da `canManageGriglia()`,
`GRIGLIA_ADMIN_GATE` o `GRIGLIA_ADMINS`. Vedi [accessi, amministratori e modalità](configuration/access.md).

### Dove finiscono i miei dati?

Nel database e nello storage della tua applicazione: task, note, allegati, domande, risultati, tempi di lavoro
e token. Griglia non manda niente da nessuna parte, salvo le notifiche o il provider AI che configuri tu.

## Lavorare con gli agenti

### Possono lavorare più agenti insieme?

Sì. Ogni lista e ogni task possono essere assegnati a un agente, `griglia:check --agent=<chiave>` mostra solo
il lavoro di quell'agente e un task preso altrove viene segnalato come occupato. Vedi
[due agenti insieme](agent/concurrency.md).

### L'agente può lavorare mentre non ci sono?

Sì, con un **worker persistente**: un piccolo processo sull'host, pubblicato da
`vendor:publish --tag=griglia-scripts` e di solito avviato come servizio utente systemd, che interroga la
board e apre una nuova sessione non interattiva dell'agente ogni volta che un task è da lavorare. Un'istanza
per chiave agente, un lock suo, e le richieste di stop vengono rispettate. Vedi
[worker persistenti](agent/workers.md).

### Come faccio a sapere che l'agente ha chiesto qualcosa o ha finito?

Te lo dice la board: campanella in-app, Web Push sui dispositivi che abiliti e mail se c'è un mailer
configurato — con un link diretto che apre il task. I due eventi seguono le impostazioni `notify_on_done` e
`notify_on_question`, che zittiscono sia la board sia il canale dell'agente. Vedi
[notifiche](features/notifications.md).

### Quanto costa in token, e come ne consumo di meno?

Griglia non chiama nessun modello al posto tuo: i token sono quelli del tuo agente. Registra quelli che
l'agente riporta chiudendo un task e li trasforma in un costo su `/stats`, con il prezzo per milione di token
che imposti in Impostazioni → App. Per consumarne meno ci sono le impostazioni ⚡ *Ottimizzazione*, che
regolano le abitudini dell'agente: output dei comandi compatto, modalità stringata, percentuale aggiornata
solo a bordo di altri comandi. Vedi [statistiche](agent/stats.md) e il
[riferimento delle impostazioni](reference/settings.md).
