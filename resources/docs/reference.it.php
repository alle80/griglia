...
return [
    ...
    'text' => [
        ...
        'Models the board may pick for a task, per agent: GRIGLIA_AGENT_MODELS="claude:opus,sonnet;codex:gpt-5" (a bare list «opus,sonnet» offers the same models to every agent; «alias=Label» renames one in the UI). Empty = no model picker: the agent CLI keeps its own default.' => 'I modelli che la board può scegliere per un task, agente per agente: GRIGLIA_AGENT_MODELS="claude:opus,sonnet;codex:gpt-5" (un elenco senza agente «opus,sonnet» vale per tutti; «alias=Etichetta» rinomina una voce nell\'interfaccia). Vuoto = nessuna scelta del modello: la CLI dell\'agente tiene il proprio default.',
        'Reasoning efforts, same shape as `agent_models`, e.g. GRIGLIA_AGENT_EFFORTS="claude:low,medium,high,xhigh,max". Empty = no effort picker.' => 'Gli effort di ragionamento, stessa forma di `agent_models`, per esempio GRIGLIA_AGENT_EFFORTS="claude:low,medium,high,xhigh,max". Vuoto = nessuna scelta dell\'effort.',
        'Which model the agent CLI (or the worker: GRIGLIA_WORKER_MODEL) already starts with, same shape as `agent_models`: GRIGLIA_AGENT_DEFAULT_MODEL="claude:opus;codex:gpt-5", or a bare «opus» for every agent. The board only NAMES it, so that a list or a task that chose nothing reads «Default (opus)» instead of a bare «CLI default»; it never sends it, so each worker keeps its own default.' => 'Con quale modello parte già la CLI dell\'agente (o il worker: GRIGLIA_WORKER_MODEL), stessa forma di `agent_models`: GRIGLIA_AGENT_DEFAULT_MODEL="claude:opus;codex:gpt-5", oppure un «opus» senza agente per tutti. La board lo NOMINA soltanto, così una lista o un task che non ha scelto niente legge «Predefinito (opus)» invece di un generico «Predefinito CLI»; non lo invia mai, quindi ogni worker tiene il proprio default.',
        'Reasoning effort the agent CLI already starts with (GRIGLIA_WORKER_EFFORT), same shape and same use: named in the pickers, never sent, e.g. GRIGLIA_AGENT_DEFAULT_EFFORT="claude:high;codex:medium".' => 'Con quale effort di ragionamento parte già la CLI dell\'agente (GRIGLIA_WORKER_EFFORT), stessa forma e stesso uso: nominato nelle tendine, mai inviato, per esempio GRIGLIA_AGENT_DEFAULT_EFFORT="claude:high;codex:medium".',
